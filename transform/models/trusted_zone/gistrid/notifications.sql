...
renamed as (
    select
        ...
        {{ adapter.quote("date du consentement français") }} as date_consentement_français,
        ...
        row_number() over (partition by {{ adapter.quote("numéro de notification") }} order by  {{ adapter.quote("annee") }} desc) as rn
    from source
)
select numero_notification,
type_dossier,
etat_dossier,
departement_dossier,
nom_notifiant,
