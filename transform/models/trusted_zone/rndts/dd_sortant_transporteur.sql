with source as (
    select * from {{ source('raw_zone_rndts', 'dd_sortant_transporteur') }}
),

renamed as (
    select
        {{ adapter.quote("dd_sortant_id") }},
        {{ adapter.quote("dd_sortant_created_year_utc") }},
