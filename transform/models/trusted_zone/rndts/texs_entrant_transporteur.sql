with source as (
    select * from {{ source('raw_zone_rndts', 'texs_entrant_transporteur') }}
),

renamed as (
    select
        {{ adapter.quote("texs_entrant_id") }},
        {{ adapter.quote("texs_entrant_created_year_utc") }},
