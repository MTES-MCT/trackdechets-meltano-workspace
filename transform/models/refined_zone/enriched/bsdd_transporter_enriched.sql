...
with filtered_data as ( -- new bsdd_transporters lines
    ...
    where bt.updated_at >= (select max(updated_at) from refined_zone_enriched.bsdd_transporter_enriched)
),
filtered_enriched_data as ( -- existing transporter_enriched_line with updated_data
    select
        t.id,
        t.number,
        ...
        b.quantity_refused,
        b.processing_operation_done
    from refined_zone_enriched.bsdd_transporter_enriched t
    inner join trusted_zone_trackdechets.bsdd as b on t.form_id = b.id and t.bordereau_updated_at<b.updated_at
),
joined_data as (
select
    *
...
final_data as (select
	*,
	row_number() over (partition by id order by greatest(updated_at,bordereau_updated_at)) as rn
from joined_data)
select
    *
from final_data
