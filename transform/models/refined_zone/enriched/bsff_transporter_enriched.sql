...
select 
	...
	max(b.transporter_company_vat_number) as transporter_company_vat_number,
	max(b.transporter_company_address) as transporter_company_address,
	max(b.transporter_company_contact) as transporter_company_contact,
	...
	bool_and(b.transporter_recepisse_is_exempted) as transporter_recepisse_is_exempted,
	max(b.transporter_recepisse_number) as transporter_recepisse_number,
	max(b.transporter_recepisse_department) as transporter_recepisse_department,
	max(b.transporter_recepisse_validity_limit) as transporter_recepisse_validity_limit,
	max(b.transporter_transport_mode) as transporter_transport_mode,
	max(b.transporter_transport_plates) as transporter_transport_plates,
	max(b.transporter_transport_taken_over_at) as transporter_transport_taken_over_at,
	max(b.transporter_transport_signature_author) as transporter_transport_signature_author,
	max(b.transporter_tranport_signature_date) as transporter_tranport_signature_date,
...
from bsff_transporter_with_bsff b
left join {{  ref('bsff_packaging') }} bp on b.bsff_id=bp.bsff_id
group by 1