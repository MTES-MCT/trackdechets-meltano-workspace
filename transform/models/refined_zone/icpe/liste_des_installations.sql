...
from 
    td_installations ti 
    full outer join gun_installations gi on ti.siret=gi.siret
where char_length(coalesce(ti.siret,gi.siret))=14 and coalesce(ti.siret,gi.siret) is not null
)
select 
    jd.siret,
    coalesce(
    ...
    jd.rubriques
from joined_data jd
left join {{ ref('company') }} c on jd.siret = c.siret
left join {{ ref('stock_etablissement') }} se on jd.siret=se.siret
left join
