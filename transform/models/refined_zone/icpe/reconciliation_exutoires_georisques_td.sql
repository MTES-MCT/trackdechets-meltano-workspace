...
select
        ...
        array_agg(distinct rubrique || coalesce ('-' || alinea, '')) as rubriques_autorises
from
        {{ref('icpe_siretise')}} is2
where
        ((rubrique in ('2770', '2790'))
        or (rubrique = '2760'
            and alinea = '1'))
    and (en_vigueur is true
...
from
        {{ref('bsdd')}} b
where
        b.processing_operation_done not in ('D13', 'D 13', 'D14', 'D 14', 'D15', 'D 15', 'R12', 'R 12', 'R13', 'R 13')
group by
        siret
order by
        siret
...
select
        coalesce(icpe_data.siret, bsdd_data.siret, bsda_data.siret, c.siret) as siret,
        max(coalesce(icpe_data.nom_etablissement, c.name, bsdd_data.company_name, bsda_data.company_name)) as nom_etablissement,
        max(case
...
left join trusted_zone_trackdechets.company c 
 on
        coalesce(icpe_data.siret, bsdd_data.siret, bsda_data.siret)= c.siret
group by
        coalesce(icpe_data.siret, bsdd_data.siret, bsda_data.siret, c.siret)