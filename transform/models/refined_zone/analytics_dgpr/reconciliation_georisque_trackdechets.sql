...
with icpe_data as (
    select
        ...
        array_agg(distinct rubrique || coalesce(
            '-' || alinea,
            ''
...
select
    ...
    )::bool as "inscrit_sur_trackdechets",
    coalesce(
        icpe_data.siret,
        bsdd_data.siret,
        bsda_data.siret,
    ...
    max(
        icpe_data.codes_s3ic
    )       as codes_s3ic,
    max(
        icpe_data.rubriques_autorises
    )       as rubriques_autorises,
    max(
        bsdd_data.nombre_bordereaux
    )       as "num_bsdd",
    max(
        bsdd_data.quantite
    )       as "tonnage_bsdd",
    max(
        bsda_data.nombre_bordereaux
    )       as "num_bsda",
    max(
        bsda_data.quantite
    )       as "tonnage_bsda"
